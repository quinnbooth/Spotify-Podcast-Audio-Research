...
# Define the breaks for the ratings bins
rating_breaks <- seq(min(data3$apple_ratings), max(data3$apple_ratings), length.out = 11)

# Discretize the ratings into 10 bins
data3$rating_bins <- cut(data3$apple_ratings, breaks = rating_breaks, include.lowest = TRUE, labels = FALSE)

# Define a color palette with 10 colors
my_palette <- inferno(10)

# Create the scatterplot with the colored points
gg3 <- ggplot(data = data3, aes(x = jitterLocal_sma3nz_amean, y = apple_ratings, color = rating_bins)) +
  geom_point() + 
...
# Add a legend for the rating bins with a custom name
gg3 <- gg3 + scale_color_gradientn(colors = my_palette, 
                                   limits = c(1, 10), 
                                   breaks = 1:10, 
                                   labels = rating_breaks[-1],
                                   name = "Rating Bins")
gg3
```
...
gg3 <- ggplot(data=data3,aes(x=data3$shimmerLocaldB_sma3nz_amean, y=data3$apple_ratings, color=data3$rating_bins)) +
  geom_point() + 
  geom_smooth(method="lm") +
  scale_y_continuous(trans='log10') +
...
gg3 <- ggplot(data=data3,aes(x=data3$HNRdBACF_sma3nz_amean, y=data3$apple_ratings, color=data3$rating_bins)) +
  geom_point() + 
  geom_smooth(method="lm") +
  scale_y_continuous(trans='log10') +
...
gg3 <- ggplot(data=data3,aes(x=data3$F0semitoneFrom27.5Hz_sma3nz_amean, y=data3$apple_ratings, color=data3$rating_bins)) +
  geom_point() + 
  geom_smooth(method="lm") +
  scale_y_continuous(trans='log10') +
...
gg3 <- ggplot(data=data3,aes(x=data3$loudness_sma3_amean, y=data3$apple_ratings, color=data3$rating_bins)) +
  geom_point() + 
  geom_smooth(method="lm") +
  scale_y_continuous(trans='log10') +
